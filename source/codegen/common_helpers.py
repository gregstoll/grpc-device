...
def is_const_ptr(dataType):
    return dataType.startswith('const') and dataType.endswith("*")
...
def is_string_arg(parameter):
  return parameter['type'] in ['ViChar[]', 'ViInt8[]', 'ViUInt8[]', 'char[]']
...
def is_streaming_out(function_data):
    return function_data.get('stream_out', False)


def is_streaming_in(function_data):
    return function_data.get('stream_in', False)
