...
def get_response_param(method_name, function):
    response_type = get_response_type(method_name)
    is_streaming = common_helpers.is_streaming_out(function)
...
def get_request_param(method_name, function):
    request_type = get_request_type(method_name)
    is_streaming = common_helpers.is_streaming_in(function)
    return f'::grpc::ServerReader<{request_type}>* reader' if is_streaming else f'const {request_type}* request'
